...
class ElixirLanguageServer {
  ...
  start(path) {
    ...
    var client: any = new LanguageClient(
          ...
          client
            ...
            .then((result) => {
              editor.edit((edit) => {
                result.map((r) => {
                  edit.replace(
                    lspRangeToRange(editor.document, r.range),
                    r.newText
                  );
                });
              });
...
function rangeToLspRange(document: TextDocument, range: Range) {
...
function lspRangeToRange(document: TextDocument, range) {
  const fullContents = document.getTextInRange(new Range(0, document.length));
  let rangeStart = 0;
  let rangeEnd = 0;
  let chars = 0;
